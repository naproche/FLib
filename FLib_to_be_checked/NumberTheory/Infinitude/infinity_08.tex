...
\definitionitem DefEmp.$\emptyset$ is a set that has no elements.
Let $X$ is {\bf empty} stand for $X = \emptyset$.
Let $X$ is {\bf nonempty} stand for $X \neq \emptyset$.

\definitionitem DefSub.  
...
\definitionitem $\Seq{m}{n} = \{ i \in \NN | m \leq i \leq n \}$.
...
\definitionitem $f$ {\bf lists} $X$ in $n$ steps iff 
$\text{dom} f = \Seq{1}{n}$ and $X = \Set{f}{1}{n}$.

\definitionitem $X$ is {\bf finite} iff there is a function
$f$ and a number $n$ such that $f$ lists $X$ in $n$ steps.

\definitionitem $X$ is {\bf infinite} iff $X$ is not finite.
\end{enumerate}
\end{definitions}
