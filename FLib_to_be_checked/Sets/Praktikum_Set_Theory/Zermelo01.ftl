...
\begin{definition} [30] $\rm{Succ}(x) = x \cup \{x\}$.
\end{definition}

\begin{lemma} $\rm{Succ}(x)$ is a set.
\end{lemma}
\end{forthel}
...
$F"A = \{F[u]\mid u \in A \text{ and } u \in \rm{Dom}(F)\}$.
\end{definition}

\begin{definition} [15b] $\rm{range}(F) = F " \rm{Dom}(F)$.
\end{definition}
\end{forthel}
...
$\rm{Dom}(F) = A$ and $\rm{range}(F) \subseteq B$.
...
function $F$ such that $\rm{Dom}(F) = A$ and
$\rm{range}(F) = B$.
\end{definition}
...
$F * v = \{u \mid u \in \rm{Dom}(F) \text{ and } F[u] = v\}$.
\end{definition}

\begin{theorem} Let $F$ be an injective function from $A$ to $x$.
Then $A$ is a set.
\end{theorem}
\begin{proof} $\rm{range}(F) \subseteq x$.
$\rm{range}(F)$ is a set. Indeed 
$\rm{range}(F) = x \cap \rm{range}(F)$.
Define $G[v] = 
\text{ choose } u \in F*v \text{ in } u \text{ for } v \text{ in } \rm{range}(F)$.
$\rm{range}(G) = A$.
\end{proof}
\end{forthel}
...
\begin{theorem} [38b] $\rm{Succ}(\alpha)$ is an ordinal. \end{theorem}
...
\begin{theorem} [42a] $\alpha < \rm{Succ}(\alpha)$.
\end{theorem}

\begin{theorem} [42b] If $\beta < \rm{Succ}(\alpha)$ 
then $\beta = \alpha$ or $\beta < \alpha$.
\end{theorem}

\begin{definition} [43a] A \emph{successor ordinal} is 
an ordinal $\alpha$ such that
$\exists \beta \ \alpha = \rm{Succ}(\beta)$.
\end{definition}
...
\begin{forthel}
\begin{theorem} [Exercise15] $\rm{Ord}$ is not a set.
\end{theorem}
\begin{proof}
Assume the contrary.
$\rm{Ord}$ is transitive and every element of 
$\rm{Ord}$ is transitive.
$\rm{Ord} \in \rm{Ord}$.
\end{proof}
\end{forthel}
...
Define $F[\alpha] = \text{ case } x \setminus (F " \alpha) = \emptyset \rightarrow x,
                  \text{ case } (x \setminus (F " alpha)) \neq \emptyset \rightarrow 
\text{ choose an element } v \text{ of } x \setminus (F " alpha) 
\text{ in } v
\text{ for } \alpha \text{ in } \rm{Ord}$. 

(1) There exists $\alpha$ such that $F[\alpha] = x$.
  Proof. Assume the contrary.
  
  (1a) For all $\alpha \in \rm{Ord} \ F[\alpha] \in x \setminus (F " \alpha)$.
  
  (1b) $F : \rm{Ord} \rightarrow x$.
  
  (1c) $F$ is an injective function from $\rm{Ord}$ to $x$.
    Proof. Let $\beta,\gamma \in \rm{Ord}$. 
    Assume $\beta \neq \gamma$.
    Then $F[\beta] \neq F[\gamma]$.
      Proof. Case $\beta < \gamma$. 
