...
Lemma 2102. Let x be inductive and x \subseteq omega. Then x = omega.
...
Proof.
  ...
  For all u,v \subseteq x if u \subseteq v then h[u] \subseteq h[v]. h is a function from \Pow(x) to
  \Pow(x).

[depthlimit 1][timelimit 9]

  There is w \subseteq x such that h[w] = w (by 4008).

[/depthlimit][/timelimit]
  
  Take w \subseteq x such that h[w] = w.

  (1) w = h[w] iff x \setminus w = g^[y \setminus f^[w]].

  Define F[u] = f[u] for u in w. g^{-1} is a bijective function from \range(g) to y. x \setminus w =
  g^[y \setminus f^[w]] \subseteq \range(g) (by 1). Define G[u] = g^{-1}[u] for u in x \setminus w.
  ...
  Let us show that for all u,v \in x if u \neq v then H[u] \neq H[v].
    ...
    Case u \in w and v \in x \setminus w.
      H[u] = F[u] and H[v] = G[v]. v \in g^[y \setminus f^[w]] (by 1). We have G[v] \in y \setminus
      F^[w]. Hence G[v] \neq F[u].
    end.

    Case u \in x \setminus w and v \in w.
      H[u] = G[u] and H[v] = F[v]. u \in g^[y \setminus f^[w]] (by 1). We have G[u] \in y \setminus
      f^[w]. Hence G[u] \neq F[v].
    end.
  end.
...
Proof.
  ...
  proof.
    ...
    Take z = ((h^{-1} \circ f) \circ g)^[\card(x)]. Then \card(x) = \card(z). Hence \card(x)
    \subseteq \card(y).
  end.
qed.
...
Definition. CardsOfCofSubsets(lambda) = {\card(x) | x \subseteq lambda and x is cofinal in lambda}.

# This lemma is necessary to fulfill the assumptions of the definitions of \min in the next
# definition.
Lemma. CardsOfCofSubsets(lambda) is nonempty and CardsOfCofSubsets(lambda) \subseteq \Ord.
Proof.
  lambda is cofinal in lambda. Indeed for all alpha \in lambda alpha + 1 \in lambda.
  Hence CardsOfCofSubsets(lambda) is nonempty.
qed.

Definition. The cofinality of lambda is \min(CardsOfCofSubsets(lambda)).
...
Lemma 8005. There is a cofinal subset x of lambda such that \cof(lambda) = \otp(x).
Proof.
  Take a cofinal subset x of lambda such that \card(x) = \cof(lambda). \otp(x) \geq \card(x).

  Let us show that \otp(x) \leq \card(x).
    Take a bijective function f from \card(x) to x. Define h[i] = {f[j] | j \in i} for i in \card(x).
    Define g[i] = \bigcup h[i] for i in \card(x).

    g is a function from \card(x) to lambda.
    proof.
      Let us show that for all i \in \card(x) g[i] \in lambda.
        Let i \in \card(x). For all j \in \card(x) h[j] \subseteq lambda. For all j \in \card(x)
        \bigcup h[j] is an ordinal.

        Hence for all j \in \card(x) g[j] \in lambda + 1.
				proof.
        ...
        proof.          
          Let us show by induction on alpha that for all alpha \in \card(x) g[alpha] \neq lambda.
            ...
            Case alpha is a limit ordinal.
              ...
              \card(h[alpha]) \leq \card(alpha).
              proof.
                alpha \subseteq \card(x). Define k[beta] = f[beta] for beta in alpha. k is a
                ...
                \neq 0.Therefore the thesis (by 4024).
              end.

              \card(alpha) < \card(x). Indeed alpha < \card(x). Thus \card(h[alpha]) < \card(x).
              Hence \card(h[alpha]) < \card(x) = \cof(lambda). For all cofinal subsets z of lambda
              \card(x) \leq \card(z). Contradiction. Indeed not \card(x) \leq \card(h[alpha]).
            end.
          end.
        end.
      end.
    ...
    proof.
      ...
      \subseteq g[j]. g[i], g[j] \in lambda. Thus g[i] \leq g[j].
    end.
