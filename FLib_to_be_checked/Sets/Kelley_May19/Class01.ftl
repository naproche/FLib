...
Signature 48. Let x,y be ssets. [x,y] is a thing.

# Definition. An ordered pair is a thing u such that 
# there exist ssets v, w such that u = [v,w].
...
# Axiom 49b. If [x,y] is not a sset then [x,y] = UU.
...
# Theorem 55. If x and y are ssets and [x,y] = [u,v] then
# x = u and y = v.
...
Axiom 55. If x,y,u,v are ssets and [x,y] = [u,v] then
x = u and y = v.

Let i,j,k stand for ssets.

[synonym relation/-s]
Definition 56. A relation is a set r such that for
each element p of r there exist i and j such that p = [i,j].

Let r,s,t stand for relations.

# Definition 57. r \circ s =
# {[i,k] | i,k are ssets and there exists j such that
# [i,j] \in s and [j,k] \in r}.
...
# Proof. (r \circ s) \circ t \subset r \circ (s \circ t) (by # #Definition).
# r \circ (s \circ t) \subset (r \circ s) \circ t.
# qed.

Definition 60. r^{-1} = {[j,i] | i,j are ssets and [i,j] \in r}.

Theorem 61. (r^{-1})^{-1} = r.
Proof. (r^{-1})^{-1} \subset r.
r \subset (r^{-1})^{-1}.
qed.
