...
Axiom.
card(M) =< card(N) iff there exists a function f such that Dom(f) = N and f^[N] = M.
...
Succ(A) is regular.
Proof.
Assume the contrary. Take a cofinal subset x of Succ(A) such that card(x) != Succ(A). Then card(x) =< A. Take a function f that is surjective from A onto x.
Define g[i] = (Choose a function h that is surjective from A onto i in h) for i in Succ(A).
...
Let us show that h is surjective from Prod(A,A) onto Succ(A).
	...
	proof.
		Let n be an element of Succ(A). Take an element xi of A such that n < f[xi]. Take an element zeta of A such that g[f[xi]][zeta] = n.
	...
	proof.
		Let n be an element of h^[Prod(A,A)]. We can take elements a,b of A such that n = h[(a,b)]. Therefore the thesis (by Transitivity). end.
	end.
