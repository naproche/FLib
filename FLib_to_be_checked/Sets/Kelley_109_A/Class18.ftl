...
Axiom I. For each x for each y x = y iff for each class z z \in x iff z \in y.
...
Definition 22. \bigcap x = {set u | for each y if y \in x then u \in y}.

Definition 23. \bigcup x = {set u | for some y (y \in x and u \in y)}.
...
Theorem 26a. 0 \subclass x.
Theorem 26b. x \subclass UU.
...
Theorem 31a. If x \subclass y then \bigcup x \subclass \bigcup y.
Theorem 31a. If x \subclass y then \bigcap y \subclass \bigcap x.

Theorem 32a. If x \in y then x \subclass \bigcup y.
Theorem 32b. If x \in y then \bigcap y \subclass x.
...
Theorem 38b. If x is a set then y \subclass x iff y \in 2^x.

Definition. RR = {set x | x \notin x}.

Theorem. RR is not a set.
# Proof. Let RR be a set.
# RR \in RR <=> RR \notin RR.
# Contradiction.
# qed.
...
Proof. Let x be a set. Then <x> \subclass 2^x. 2^x is a class.
qed.
...
Theorem 44d. If x is not a set then \bigcup <x> = UU.
...
Theorem 46b. If x is a set and y is a set then
z \in <x,y> iff z=x or z=y.
Theorem 46c. <x,y> = UU iff x is not a set or y is not a set.
...
Theorem 47c. If x is not a set or y is not a set then
\bigcap <x,y> = 0.
Theorem 47d. If x is not a set or y is not a set then
\bigcup <x,y> = UU.
...
Theorem. If x is not a set or y is not a set then
...
Theorem 54a. If x and y are sets then 1st [x,y] = x.
Theorem 54b. If x and y are sets then 2nd [x,y] = y.
...
Theorem 54c. If x is not a set or y is not a set then
