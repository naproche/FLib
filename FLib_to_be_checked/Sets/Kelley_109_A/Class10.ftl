...
# We allow elements of sets to be structureless
# atoms which we call maob, for "mathematical object".
...
# thing is the temporary denotation of "mathematical objects"
# mob
[thing/-s]
Signature. A thing is a notion. 

Let a, b, c stand for things.

# Axiom I. Axiom of extent.
Axiom I. For each x for each y x = y iff for each thing z z \in x iff z \in y.

# These are the sets of KM
[sset/-s]
Definition 1. A sset is a set that is a thing.

# Elementary Algebra of Classes

Definition 2. x \cup y = { thing u | u \in x or u \in y }.

Definition 3. x \cap y = { thing u | u \in x and u \in y }.
...
Definition 18. UU = {thing u | u = u}.
Let the universe stand for UU.

Theorem 19. x \in UU iff x is a thing.
...
Definition 22. \bigcap x = {thing u | for each sset y if y \in x then u \in y}.

Definition 23. \bigcup x = {thing u | for some sset y (y \in x and u \in y)}.
...
Theorem 35. If x has an element that is a sset 
then \bigcap x is a sset.
...
Definition 40. <x> = {thing z | if x \in UU then z = x}.
Let the singleton of x stand for <x>.

Theorem 41. If x is a thing then for each y y \in <x> iff y = x.

Theorem 42. If x is a thing then <x> is a sset.
Proof. Let x be a thing. Then <x> \subset 2^x. 2^x is a set.
qed.

Theorem 43. <x> = UU iff x is not a thing.

Theorem 44a. If x is a thing then \bigcap <x> = x.
Theorem 44b. If x is a thing then \bigcup <x> = x.
# Theorem 44c. If x is not a thing then \bigcap <x> = 0.
# Theorem 44d. If x is not a thing then \bigcup <x> = UU.

# [/prove]

Axiom IV. If x is a sset and y is a sset then x \cup y is a sset.

Definition 45. Let x,y be things. <x,y> = <x> \cup <y>.
Let the unordered pair of x and y stand for <x,y>.

Theorem 46a. If x is a thing and y is a thing then <x,y> is a sset.
Theorem 46b. If x is a thing and y is a thing then
z \in <x,y> iff z=x or z=y.
# Theorem 46c. <x,y> = UU iff x is not a thing or y is not a thing.
...
# Theorem 47c. If x is not a thing or y is not a thing then
# \bigcap <x,y> = 0.
# Theorem 47d. If x is not a thing or y is not a thing then
# \bigcup <x,y> = UU.

# Ordered Pairs; Relations

Definition 48. Let x,y be things. [x,y] = <<x>,<x,y>>.
Let the ordered pair of x and y stand for [x,y].

Theorem 49a. If x,y are things then [x,y] is a sset.
# Theorem 49b. If [x,y] is not a thing then [x,y] = UU.



Theorem 50a. If x and y are things then 
  \bigcup [x,y] = <x,y> and
  \bigcap [x,y] = <x> and
...
# Theorem. If x is not a thing or y is not a thing then
...
Theorem 54a. If x and y are things then 1st [x,y] = x.
Theorem 54b. If x and y are things then 2nd [x,y] = y.
Proof. Let x and y be things.
...
# Theorem 54c. If x is not a thing or y is not a thing then
# 1st [x,y] = UU and 2nd [x,y] = UU.

Theorem 55. If x,y,r,s are things and [x,y] = [r,s] then
x = r and y = s.
