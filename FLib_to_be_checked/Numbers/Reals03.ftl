...
Proof. Let E be a subset of RR such that E is bounded below.
...
Then E = NN.
Proof.
Assume E [[ NN and 1 << E and for all x << E x + 1 << E. 
Let n << NN.
Let us show that n << E.
Assume the contrary.
Let D = { m << NN | not m << E}.
D is nonempty. D is bounded below.
Take a greatest lower bound a of D.
Let us show that a+1 is a lower bound of D.
Let x << D. x - 1 << ZZ.
Case x - 1 < 0. Then 0 < x < 1. Contradiction. end.
Case x - 1 = 0. Then x = 1. not 1 << E. Contradiction. end.
Case x - 1 > 0. Then x - 1 << NN. x - 1 << D.
a <= x - 1. 
a + 1 <= (x - 1) + 1 = x.
end.
end.
