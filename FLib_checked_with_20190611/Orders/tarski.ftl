[synonym set/-s] [synonym subset/-s] 
[synonym element/-s] [synonym belong/-s]
...
[synonym bound/-s] [synonym supremum/-s] 
[synonym infimum/-s] [synonym lattice/-s]
...
Definition RanSort. Let f be a function.  Ran(f) =  {f[x] | x << Dom(f)}.
...
Definition DefMonot.
    f is monotone on S iff f is on S and for all x,y << Dom(f)
        x <= y  =>  f[x] <= f[y].


Theorem Tarski.
    Let U be a complete lattice.
    Let f be monotone on U.
