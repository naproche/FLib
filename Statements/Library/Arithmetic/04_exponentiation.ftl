...
Axiom 0102. n^{m + 1} = n^{m} \cdot n.


Let the square of x stand for x^{2}. Let the cube of x stand for x^{3}.


Proposition 0403. n^{m} is a natural number.
...
Proof.
  ...
  proof.
    ...
    proof.
      Let x be a natural number. We have x^{y + 1} = x^{y} \cdot x. Hence
      x^{y + 1} is a natural number.
    end.
  end.
...
Proposition 0404. Assume that n \neq 0. Then 0^{n} = 0.
...
Proof.
  [prove off]
  # Define P(z) iff x^{y + z} = x^{y} \cdot x^{z} for all natural numbers x,y.
...
Proof.
  [prove off]
  # Define P(z) iff (x^{y})^{z} = x^{y \cdot z} for all natural numbers x,y.
...
Proof.
  [prove off]
  # Define P(z) iff (x \cdot y)^{z} = x^{z} \cdot y^{z} for all natural numbers
  # x,y.
...
Proposition 0411. n^{m} = 0 iff n = 0 and m \neq 0.

Proof.
  If n^{m} = 0 then n = 0 and m \neq 0.
  proof.
    [prove off]
    # Define P(y) iff for all natural numbers x if x^{y} = 0 then x = 0 and
    # y \neq 0.
    ...
    proof.
      ...
      For all natural numbers x if x^{y + 1} = 0 then x = 0 and y + 1 \neq 0.
      proof.
        Let x be a natural number. Assume x^{y + 1} = 0. Then 0 = x^{y + 1} =
        x^{y} \cdot x. Hence x^{y} = 0 or x = 0. We have y + 1 \neq 0 and if
