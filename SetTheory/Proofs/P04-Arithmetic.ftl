[read Forthel-Dateien/SetTheory/Library/L03-Von_Neumann_Hierarchy.ftl]
...
Axiom. Let alpha, beta be ordinals. Let beta /in /Succ. Let gamma = beta --.
...
Proof.
  ...
  Proof by induction.
    ...
    Proof.
      ...
      Case gamma /in /Lim.
        Then alpha + gamma = {zfset x | exists delta /in gamma (x /in alpha + delta)}.
        Let beta /in gamma.
...
Proof by induction.
  ...
  Proof.
    ...
    Case alpha /in /Lim.
      0 * alpha = {zfset x | exists beta /in alpha (x /in (0 * beta))}.
    end.
    Case alpha = 0.
    end.
  end.
...
Proof by induction.
  ...
  Proof.
    ...
    Case alpha /in /Lim.
      1 * alpha = {zfset x | exists gamma /in alpha (x /in 1 * gamma)}.
  ...
  Proof by induction.
    ...
    Proof.
      ...
      case beta /in /Lim.
        ...
        x /in alpha * (x + 1).
        x + 1 /in beta.
        alpha * beta = {zfset x | exists gamma /in beta (x /in alpha * gamma)}.
        Then x /in alpha * beta.
      end.
      Case beta = 0.
      end.
...
Axiom. Dom(Plus) = /Ord.
Axiom Plus. Let alpha /in /Ord. Then Plus[alpha] is a set and Plus[alpha] = {ordinal beta | forall kappa /in /Cd (alpha /in kappa => beta /in kappa)}.

Lemma. Forall alpha /in /Ord Plus[alpha] /in /Cd.
Proof.
  Let alpha /in /Ord.
  Exists kappa /in /Cd (alpha /in kappa).
  Proof.
    alpha < /PP alpha.
    Then alpha /in Card(/PP alpha).
    Proof by contradiction. Assume the contrary.
      Card(alpha) /in Card(/PP alpha).
      alpha, Card(/PP alpha) /in /Ord.
      alpha /notin Card(/PP alpha).
      Then alpha = Card(/PP alpha) \/ Card(/PP alpha) /in alpha.
      Case alpha = Card(/PP alpha). 
        Then alpha /tilde /PP alpha.
        Contradiction.
      end.
      Case Card(/PP alpha) /in alpha.
        Then Card(/PP alpha) <= alpha.
        Then Card(/PP alpha) /subset Card(alpha).
        Contradiction.
      end.
    end.
  end.
  Define M = {cardinal kappa | alpha /in kappa}.
  Then M /neq /emptyset.
  Let kappa = min(M).
  M /subset /Ord.
  Then kappa /in M.
  Plus[alpha] /subset kappa.
  kappa /subset Plus[alpha].
  Proof.
    Let a /in kappa.
    Forall lambda /in M a /in lambda.
    Then forall lambda /in /Cd (alpha /in lambda => a /in lambda).
    a is an ordinal.
    Plus[alpha] = {ordinal beta | forall lambda /in /Cd (alpha /in lambda => beta /in lambda)} (by Plus).
    Then a /in Plus[alpha].
  end.
  Then Plus[alpha] = kappa.
qed.
...
Proof. n /in Plus[n]. 
  ...
  Proof. 
    ...
    Proof.
      ...
      Plus[n] = {ordinal beta | forall kappa /in /Cd (n /in kappa => beta /in kappa)}.
      Then m /in Plus[n].
    end.
    n /in Plus[n].
  end.
...
Lemma. Let alpha /in /Lim. Then Alef[alpha] = /bigcup Alef^[alpha].
Proof.
  Alef[alpha] /subset /bigcup Alef^[alpha].
  /bigcup Alef^[alpha] /subset Alef[alpha].
  Proof.
    Let a /in /bigcup Alef^[alpha].
    Take a zfset x such that x /in Alef^[alpha] /\ a /in x.
    Take an ordinal b such that b /in alpha /\ x = Alef[b].
    Then a /in Alef[b].
    Alef[alpha] = {zfset y | exists beta /in alpha y /in Alef[beta]}.
    Then a /in Alef[alpha].
  end.
qed.
...
Proof by induction.
  ...
  Proof.
    ...
    Case alpha /in /Lim.
      ...
      Alef[alpha] = {zfset x | exists gamma /in alpha x /in Alef[gamma]}.
      Then Alef[beta] /in Alef[alpha].
    end.
    Case alpha = 0.
    end.
...
Lemma. Forall alpha, beta /in /Ord (alpha /in beta iff Alef[alpha] /in Alef[beta]).


Lemma. Forall alpha, beta /in /Ord (beta /subset alpha iff Alef[beta] /subset Alef[alpha]).
  ...
  Case kappa /in Alef[kappa].
    ...
    Case Alef[gamma] /in kappa.
      ...
      Proof. Take a cardinal alefgamma such that Alef[gamma] = alefgamma.
        Plus[alefgamma] = {ordinal beta | forall lambda /in /Cd (alefgamma /in lambda => beta /in lambda)}.
      end.
