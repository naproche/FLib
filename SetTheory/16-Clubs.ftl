...
# We want to show that the filter Club(kappa) is closed under certain intersections, for this we need to define
# sequences of subsets in Club(kappa). Later-on we want to show similiar results for non-stationary subsets.
# For this we define for an arbitrary class C a sequence of length lambda in C; this notion then can be applied to
# sequences in Club(kappa), in /PP kappa, in NS(kappa), and so on.
...
Lemma. Let f be a zffunction. Let ran(f) /subset Dom(f). Let n /in /NN. Let n /neq 0. Let x /in Dom(f). Then (x /in Dom(f^(n-1))) /\ (f^(n-1))[x] /in Dom(f) /\ (f^(n))[x] = f[((f^(n-1))[x])].
Proof.
