...
Proof.
  ...
  Proof.
    y, /bigcup x /in /Ord.
    Then y /in /bigcup x \/ /bigcup x /in y \/ y = /bigcup x (by TotalOrder).
    y /subset (/bigcup x).
  end.
...
Proof.
  ...
  Proof.
    Let x /in ran(TCol R). 
    Take a set y such that y /in reldomain(R)  /\ x = (TCol R)[y].
    Let x1 /in x. 
...
Proof.
  ...
  Forall x /in ran(TCol R) Trans(x).
  Proof.
    Let x /in ran(TCol R). Take a set y such that y /in reldomain(R)  /\ x = (TCol R)[y].
    Let x1 /in x. 
    Then x1 /in (TCol R)^[sset(R,y)]. 
    Take a set y1 such that y1 /in sset(R,y) /\ x1 = (TCol R)[y1].
    Let x2 /in x1. 
