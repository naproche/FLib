...
by K. Buzzard, J. Commelin, and P. Massot \cite{Buzzard} is a milestone in bringing 
...
controlled natural language ForTheL (Formula Theory Language)
as its
input language (see also \cite{Paskevich}) which is a subset of \LaTeX{}. 
...
mostly use the E first-order prover \cite{Schulz} but also Vampire \cite{Vampire}.
...
\texttt{perfectoidrings.ftl.tex}
can be checked in the \Naproche{} program by opening the file in Isabelle generic proof assistant
\cite{Isabelle}.
\Naproche{} proof checking constitutes a major computational process: each statement generates several
first-order proof tasks each of which can take several seconds of
ATP proving. To check the whole document takes about half an hour on older laptop, 
...
Technically this may help to focus proof search on the domains under consideration.

For convenience we also state some 
obvious axioms which with some effort could be proved as lemmas. 
...
for mathematical properties (macros). These are imported as
library files which contain \Naproche{} commands of the form
\begin{verbatim}
[synonym number/-s]
\end{verbatim}
and
...
Our foundations correspond to a Kelley-Morse type set theory with atoms 
but without the axiom of choice. The axiom of infinity will be stated after
the introduction of natural numbers.
New notions and axioms will be \textit{conservative} over classical Kelley-Morse set theory \cite{Kelley} and 
can in principle be reduced to definitions and propositions within that system, as is usual when developing mathematics in set theories.
...
\subsubsection{Two Equalities}

Since \Naproche{} is weak on algebraic manipulations, we 
state and prove two complex equalities for later use.
...
\begin{axiom}[Associativity] $(x +^{G} y) +^{G} z = x +^{G} (y +^{G} z)$ for all $x,y,z \in G$.
\end{axiom} 
...
\begin{axiom}[Summation recursion]
...
\end{axiom}
\end{forthel}
The next proofs will be rather tedious since 
...
It is interesting to contrast this declarative 
natural language proof with the corresponding formal tactic proof in \verb+power_bounded.lean+:
\begin{verbatim}
...
\begin{proof}
Let $U$ be a neighborhood of $0^{R}$ in $R$.
$R$ is multiplicatively continuous at $0^{R}$ and $0^{R}$ and
$U$ is a neighborhood of $0^{R} \cdot^{R} 0^{R}$ in $R$.
[prover vampire][timelimit 10]
Take sets $V,V'$ such that
...
$R^o/a$ and $R^o/b$. To cut things short, we (slightly
miss-)use the notation
...
The present formalization has mainly been directed towards 
